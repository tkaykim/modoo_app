...
export type MessageContentType =
  ...
  | 'location_selector'   // (구버전) Print location multi-select
  | 'design_size_input'   // 크기별 디자인 개수 입력
  | 'quantity_input'      // Exact quantity number input
...
// 선호 방향 (단일 선택). 추천 엔진은 '퀄리티'면 자수 우선, 그 외는 최저가.
export type Priority = '가격' | '밸런스' | '퀄리티';

// Design type — drives print method recommendation
export type DesignType = '사진·그래픽' | '로고·텍스트';
...
// 크기별 인쇄 디자인 개수 (단가 사이즈 버킷과 1:1)
export interface DesignSizeCounts {
  '10x10': number;  // 작은 디자인 (~10cm)
  A4: number;       // 중간 (A4 이내)
  A3: number;       // 큰 (A3 이내)
}
...
export interface InquiryData {
  ...
  quantity?: QuantityOption;     // (구버전) 구간 선택 — 폴백용
  quantityExact?: number;        // 사용자가 직접 입력한 대략 수량 (견적은 이 값을 우선 사용)
  designType?: DesignType;
  colorCount?: ColorCount;        // undefined when 사진/풀그래픽 (auto 풀컬러)
  printLocations?: PrintLocation[];  // (구버전) 인쇄 위치
  designSizes?: DesignSizeCounts;     // 크기별 디자인 개수 (견적 산정에 사용)
