...
function statusLabel(status: string) {
  ...
  if (status === 'ongoing') return { text: '진행중', cls: 'text-brand' };
    ...
    return (
      <div className="min-h-screen bg-white pb-24">
        ...
        <section className="px-4 pt-5 pb-4">
          ...
          <div className="relative mt-4">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 rounded-full border border-gray-300 text-sm focus:outline-none focus:border-brand transition"
            />
          </div>
        </section>
        ...
        <section className="mt-2 border-t-8 border-gray-50">
          ...
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-brand/10 shrink-0">
              <MessagesSquare className="w-5 h-5 text-brand" />
            </span>
            <span className="flex-1 min-w-0">
              <span className="flex items-center gap-2">
                <span className="text-sm font-bold text-gray-900">채팅 상담</span>
                <span className="text-[11px] font-medium text-brand bg-brand/10 px-1.5 py-0.5 rounded">바로 답변 가능</span>
              </span>
        ...
        <section className="mt-2 border-t-8 border-gray-50">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <button onClick={() => router.push('/inquiries?tab=all')} className="text-xs text-gray-500 flex items-center gap-0.5 hover:text-brand">
        ...
        <div className="fixed bottom-0 inset-x-0 z-20 bg-white border-t border-gray-100 p-3 flex gap-2">
          <button
            onClick={() => router.push('/chat')}
            className="flex-1 py-3.5 bg-white text-brand border border-brand rounded-xl font-bold text-sm hover:bg-blue-50 transition flex items-center justify-center gap-1.5"
          >
            <MessagesSquare className="w-4 h-4" /> 채팅 상담 시작하기
          </button>
          <button
            onClick={() => router.push('/inquiries?tab=all')}
            className="flex-1 py-3.5 bg-brand text-white rounded-xl font-bold text-sm hover:bg-[#2f4584] transition flex items-center justify-center gap-1.5"
          >
      ...
      <div>
        {listLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => router.push('/inquiries/new')} className="px-6 py-3 bg-brand text-white rounded-lg text-sm hover:bg-[#2f4584] transition">
              첫 문의 등록하기
            </button>
          </div>
        ) : (
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg w-full max-w-sm text-center" onClick={(e) => e.stopPropagation()}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handlePasswordSubmit(); }}>
            <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)}
              ...
              className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-brand transition mb-3" />
            ...
            <button type="submit" disabled={isVerifying || !passwordInput.trim()}
              className="w-full py-3 bg-brand text-white rounded-lg hover:bg-[#2f4584] transition disabled:bg-gray-400 text-sm font-medium">
              {isVerifying ? '확인 중...' : '확인'}
            </button>
          </form>
        </div>
