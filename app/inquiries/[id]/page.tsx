            ...
            <div className="bg-white rounded-lg p-8 shadow-sm text-center">
              ...
              >
                ...
                <button
                  ...
                  className="w-full py-3 bg-brand text-white rounded-lg hover:bg-[#2f4584] transition disabled:bg-gray-400 disabled:cursor-not-allowed text-sm font-medium"
                >
                  {isVerifying ? '확인 중...' : '확인'}
                </button>
              </form>
                ...
                {inquiry.file_urls.map((url: string, idx: number) => {
                  ...
                  return isImg ? (
                    ...
                    <a
                      ...
                      className="flex items-center gap-2 text-sm text-brand hover:underline px-3 py-2 border border-gray-200 rounded-lg"
                    >
                ...
                return (
                  <div key={ord.orderId} className="border border-gray-200 rounded-lg p-4 flex gap-4 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-base font-bold text-brand mt-1">총 {ord.totalAmount.toLocaleString()}원</p>
                    </div>
                    <div className="shrink-0">
                      {paid ? (
                        ...
                        <a
                          href={ord.payUrl}
                          className="inline-block px-5 py-2.5 rounded-lg text-sm font-bold bg-brand text-white hover:bg-[#2f4584] transition"
                        >
                          바로 결제하기
                        </a>
                      ) : null}
          ...
          {inquiry.replies && inquiry.replies.length > 0 ? (
            <div className="space-y-4 mb-6">
              {inquiry.replies.map((reply: any) => (
                <div key={reply.id} className={`border-l-4 pl-4 py-2 ${reply.is_admin === false ? 'border-gray-300' : 'border-brand'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-medium ${reply.is_admin === false ? 'text-gray-700' : 'text-brand'}`}>
