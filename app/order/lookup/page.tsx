...
const orderStatusMap: Record<string, { label: string; color: string }> = {
  ...
  shipping: { label: '배송중', color: 'bg-indigo-100 text-brand' },
          ...
          <form onSubmit={handleLookup} className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-black text-sm focus:outline-none focus:ring-2 focus:ring-brand"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-2.5 bg-brand text-white rounded-lg font-medium hover:bg-brand-deep transition disabled:bg-gray-400 flex items-center justify-center gap-2 text-sm"
            >
              {isLoading ? (
                ...
                <div className="flex items-center gap-1">
                  {progressSteps.map((step, idx) => (
                    <div key={step.key} className="flex-1 flex flex-col items-center">
                      <div className={`w-full h-1.5 rounded-full ${idx < currentStep ? 'bg-brand' : 'bg-gray-200'}`} />
                      <span className={`text-[10px] mt-1 ${idx < currentStep ? 'text-brand font-medium' : 'text-gray-400'}`}>
                        {step.label}
                      </span>
                    </div>
                  ))}
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      {order.coupon_discount > 0 && (
                        <div className="flex justify-between">
                          <span className="text-brand">쿠폰 할인</span>
                          <span className="text-brand">-{order.coupon_discount.toLocaleString('ko-KR')}원</span>
                        </div>
                      )}
                      {workCost !== 0 && (
