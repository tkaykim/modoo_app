...
const statusConfig: Record<CoBuyRequestStatus, { label: string; icon: typeof Clock; color: string; bgColor: string }> = {
  ...
  in_progress: { label: '디자인 작업중', icon: Pencil, color: 'text-brand', bgColor: 'bg-blue-50' },
                  ...
                  <div key={status} className={`flex items-start ${isLast ? '' : 'flex-1'}`}>
                    ...
                    <div className="flex flex-col items-center" style={{ width: 28 }}>
                      ...
                      <p className={`text-[9px] mt-1.5 text-center leading-tight whitespace-nowrap ${
                        isCurrent ? 'text-brand font-semibold' : isDone ? 'text-gray-400 font-medium' : 'text-gray-400'
                      }`}>
                    ...
                    {!isLast && (
                      <div className="flex-1 flex items-center" style={{ height: 28 }}>
                        <div className={`w-full border-t-2 border-dashed mx-1 ${
                          isDone ? 'border-brand' : 'border-gray-300'
                        }`} />
                      </div>
                    )}
                  </div>
        ...
        {request.status === 'design_shared' && (
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
            ...
            <button
              ...
              className="w-full py-3.5 bg-brand text-white rounded-xl text-sm font-bold hover:bg-brand-deep disabled:opacity-50 transition flex items-center justify-center gap-2"
            >
              {isConfirming ? (
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 safe-area-inset-bottom">
          <div className="max-w-lg mx-auto flex gap-2">
            <input
              ...
              className="feedback-comment-input flex-1 px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-brand transition"
            />
            <button
              onClick={handleSendComment}
              disabled={!commentText.trim() || isSending}
              className="px-4 py-2.5 bg-brand text-white rounded-xl text-sm font-medium hover:bg-brand-deep disabled:opacity-50 transition flex items-center gap-1.5"
            >
